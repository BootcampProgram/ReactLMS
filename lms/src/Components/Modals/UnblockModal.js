...
const  UnblockStudent = () => {
    ...
    <Modal.Body>
      <Container>
        <Row className="show-grid, text-right mt-3">
          <Col>
            <Button variant="primary" className="mr-3" size="md" onClick={() => {successModal(true); UnblockStudent(); setstudentId(props.studentid)}}>Yes</Button>
  ...
  <UnblockSuccessfulModal show={UnblockSuccessfulModalShow} onHide={() => setUnblockSuccessfulModalShow(false)} studentid = {studentId}  respondmessage={respondMessage}/>
  </>
  );
}
